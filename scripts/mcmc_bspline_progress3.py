...
class BN_LTE_MCMC_BSpline_Optimized:
    ...
    def __init__(
        ...
        *,
        ...
        self.tau_phi = 15.0  # Ridge precision for phi_coeffs (large = more shrinkage/smoothness)
        ...
        self.binary_indices = [2,3,4,5,6]
        self.continuous_indices = [0, 1]
        ...
        self.K_phi = 5  # Number of B-spline basis functions for phi(z)
        ...
        self.d_total = len(self.binary_indices) + len(self.continuous_indices) * self.K_c_cont  # Always 15
        c0 = np.random.randn(self.d_total, self.K_phi) * 0.01  # (15,5)
        ...
        self.K_phi = 5  # Number of B-spline basis functions for phi(z)
        self.t_phi = _make_t(self.K_phi, self.degree)  # Knot vector for phi
       
        phi_coeffs0 = np.random.randn(self.n, self.K_phi) * 0.01  # Small random coeffs

        # self.state = StateBS(sigma2_0, beta0, gamma0, r0, z0, tau0, sigma3, rho, c0, eta0, z_time0, phi0, sigma_epsilon0)
        self.state = StateBS(sigma2_0, beta0, gamma0, r0, z0, tau0, sigma3, rho,  c0, eta0,  z_time0, phi_coeffs0,sigma_epsilon0)  # No eta, no C
        self.alpha = 1.0
        self.h.tau_phi  # Access hyper
    ...
    def _update_z_time(self, it) -> None:
        ...
        for t in range(1, self.T):
            for i in range(self.n):
                ...
                # NEW: Compute phi_basis from z_prev and mu_trans
                z_prev = self.state.z_time[t - 1, i]
                phi_basis = bspline_basis_single_row(z_prev, K=self.K_phi, degree=self.degree, t=self.t_phi)
                mu_trans = phi_basis @ self.state.phi_coeffs[i]

                b_old_beta = bspline_basis_single_row(z_current, K=self.K_beta, t=self.t_beta)  # Note: Use z_current for old basis
                b_new_beta = bspline_basis_single_row(z_prop, K=self.K_beta, t=self.t_beta)
                b_old_gamma = bspline_basis_single_row(z_current, K=self.K_gamma, t=self.t_gamma)
                b_new_gamma = bspline_basis_single_row(z_prop, K=self.K_gamma, t=self.t_gamma)
                ...
                # MODIFIED: Transition prior delta using mu_trans
                sigma_e = np.sqrt(self.state.sigma_epsilon)
                lp_old = st.norm.logpdf(z_current, loc=mu_trans, scale=sigma_e)
                lp_new = st.norm.logpdf(z_prop, loc=mu_trans, scale=sigma_e)
                lp_delta = lp_new - lp_old

                # Logdet delta (unchanged)
                B_old = self._B_from_basis_row(b_old_beta)
                B_new = self._B_from_basis_row(b_new_beta)
                _, logdet_old = np.linalg.slogdet(np.eye(self.p) - B_old)
                _, logdet_new = np.linalg.slogdet(np.eye(self.p) - B_new)
    ...
    def _update_C(self):
        """Update C column-wise: each C[:,k] ~ N(mu_post_k, Sigma_post_k) via weighted ridge regression."""
        d_total = len(self.binary_indices) + len(self.continuous_indices) * self.K_c_cont  # 15
        K_phi = self.K_phi  # 5
        Sigma_inv_base = np.zeros((d_total, d_total))
        
        # Build base precision (type-specific ridge prior)
        for j in range(d_total):
            if j < len(self.binary_indices):
                Sigma_inv_base[j, j] = 1.0 / self.tau_C_binary
            else:
                Sigma_inv_base[j, j] = 1.0 / self.tau_C_cont
        
        # For each column k, compute posterior
        for k in range(K_phi):
            # Collect y = phi_coeffs[:,k] (n,), X_i = c_i (d_total x n, but we'll sum)
            grad_ll_k = np.zeros(d_total)  # sum_i (y_{i,k} / eta_i) * c_i
            Sigma_inv_k = Sigma_inv_base.copy()  # (d_total, d_total)
            
            for i in range(self.n):
                # Get c_tensor for i (latest valid time)
                valid_times = np.where(self.valid_time_mask[:, i])[0]
                if len(valid_times) == 0:
                    c_tensor_i = self.mixed_covariate_row(self.V_norm[0, i, :])
                else:
                    t_latest = valid_times[-1]
                    c_tensor_i = self.mixed_covariate_row(self.V_norm[t_latest, i, :])
                
                y_ik = self.state.phi_coeffs[i, k]
                outer_ct = np.outer(c_tensor_i, c_tensor_i) / self.state.eta[i]
                Sigma_inv_k += outer_ct
                grad_ll_k += (y_ik / self.state.eta[i]) * c_tensor_i
            
            try:
                L = np.linalg.cholesky(Sigma_inv_k + 1e-6 * np.eye(d_total))
                mu_post_k = cho_solve((L, True), grad_ll_k)
                # Sample C[:,k]
                self.state.C[:, k] = st.multivariate_normal.rvs(
                    mean=mu_post_k, cov=cho_solve((L, True), np.eye(d_total)), random_state=self.rng
                )
            except np.linalg.LinAlgError:
                print(f"LinAlgError in _update_C for column k={k}: using prior mean 0")
                self.state.C[:, k] = np.zeros(d_total)  # Fallback to prior mean
    ...
    def _update_phi_coeffs(self) -> None:
        """Update per-subject phi_coeffs via Bayesian linear regression: z_t ~ N(Phi @ coeffs_i, sigma_epsilon)"""
        sigma_epsilon = self.state.sigma_epsilon
        tau_phi = self.h.tau_phi  # Ridge precision
        
        for i in range(self.n):
            # Collect valid consecutive pairs
            pairs = []
            for t in range(1, self.T):
                if self.valid_time_mask[t - 1, i] and self.valid_time_mask[t, i]:
                    z_prev = self.state.z_time[t - 1, i]
                    z_t = self.state.z_time[t, i]
                    pairs.append((z_prev, z_t))
            
            num_pairs = len(pairs)
            # if num_pairs == 0:
            #     # Sample from prior if no data
            #     self.state.phi_coeffs[i] = st.multivariate_normal.rvs(
            #         mean=np.zeros(self.K_phi), cov=tau_phi * np.eye(self.K_phi), random_state=self.rng
            #     )
            #     continue
            if num_pairs == 0:
                # Prior: N(c_tensor @ C, eta_i * I)
                valid_times = np.where(self.valid_time_mask[:, i])[0]
                if len(valid_times) == 0:
                    c_tensor = self.mixed_covariate_row(self.V_norm[0, i, :])
                else:
                    ...
                    c_tensor = self.mixed_covariate_row(self.V_norm[t_latest, i, :])
                mu_prior = c_tensor @ self.state.C
                cov_prior = self.state.eta[i] * np.eye(self.K_phi)
                self.state.phi_coeffs[i] = st.multivariate_normal.rvs(mean=mu_prior, cov=cov_prior, random_state=self.rng)
                continue
            
            # Stack design matrix Phi (num_pairs x K_phi)
            Phi = np.array([bspline_basis_single_row(z_prev, K=self.K_phi, degree=self.degree, t=self.t_phi) 
                            for z_prev, _ in pairs])
            y = np.array([z_t for _, z_t in pairs])  # (num_pairs,)
            
            # Posterior: MVN( (Phi^T Phi / sig_e + I/tau_phi)^{-1} (Phi^T y / sig_e), inv(...) )
            XtX = Phi.T @ Phi / sigma_epsilon + np.eye(self.K_phi) / tau_phi
            Xty = Phi.T @ y / sigma_epsilon
            
            # try:
            #     L = np.linalg.cholesky(XtX + 1e-6 * np.eye(self.K_phi))  # Stabilize
            #     mean_post = cho_solve((L, True), Xty)
            #     cov_post = cho_solve((L, True), np.eye(self.K_phi))
            #     self.state.phi_coeffs[i] = st.multivariate_normal.rvs(
            #         mean=mean_post, cov=cov_post, random_state=self.rng
            #     )
            # except np.linalg.LinAlgError:
            #     print(f"LinAlgError in _update_phi_coeffs for i={i}; using prior")
            #     self.state.phi_coeffs[i] = st.multivariate_normal.rvs(
            #         mean=np.zeros(self.K_phi), cov=tau_phi * np.eye(self.K_phi), random_state=self.rng
            #     )
            mu_prior = c_tensor @ self.state.C  # Same c_tensor as above
            prec_prior = np.eye(self.K_phi) / self.state.eta[i]
            XtX_like = Phi.T @ Phi / sigma_epsilon
            XtX = XtX_like + prec_prior
            Xty_like = Phi.T @ y / sigma_epsilon
            Xty = Xty_like + prec_prior @ mu_prior
            
            try:
                L = np.linalg.cholesky(XtX + 1e-6 * np.eye(self.K_phi))
                mean_post = cho_solve((L, True), Xty)
                cov_post = cho_solve((L, True), np.eye(self.K_phi))
                self.state.phi_coeffs[i] = st.multivariate_normal.rvs(mean=mean_post, cov=cov_post, random_state=self.rng)
            except np.linalg.LinAlgError:
                # Fallback to prior
                self.state.phi_coeffs[i] = st.multivariate_normal.rvs(mean=mu_prior, cov=self.state.eta[i] * np.eye(self.K_phi), random_state=self.rng)
    ...
    def _update_sigma_epsilon(self) -> None:
        """Update sigma_epsilon using z_t - phi(z_{t-1}) residuals."""
        total_ss = 0.0
        num_trans = 0
        for t in range(1, self.T):
            for i in range(self.n):
                if self.valid_time_mask[t - 1, i] and self.valid_time_mask[t, i]:
                    ...
                    phi_basis = bspline_basis_single_row(z_prev, K=self.K_phi, degree=self.degree, t=self.t_phi)
                    mu_trans = phi_basis @ self.state.phi_coeffs[i]
                    residual = self.state.z_time[t, i] - mu_trans
                    total_ss += residual ** 2
                    num_trans += 1
        
        if num_trans == 0:
            self.state.sigma_epsilon = st.invgamma.rvs(a=self.h.nu_epsilon, scale=self.h.lambda_epsilon)
        else:
            shape = self.h.nu_epsilon + num_trans / 2
            scale = self.h.lambda_epsilon + 0.5 * total_ss
            self.state.sigma_epsilon = st.invgamma.rvs(a=shape, scale=scale)
