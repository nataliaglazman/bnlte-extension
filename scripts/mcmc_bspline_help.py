...
def bspline_basis_single_row(var_val: float, *, K: int, degree: int = 3, t: np.ndarray = None) -> np.ndarray:
    """Compute a single row of the B-spline basis matrix (for speed)."""
    if t is None:
        n_internal = K - degree - 1
        if n_internal < 0:
    ...
    for k in range(K):
        coeff = np.zeros(K)
        coeff[k] = 1.0
        basis_row[k] = BSpline(t, coeff, degree)(var_val)
    return basis_row
...
def tensor_product_basis_row(v: np.ndarray, *, K: int, degree: int = 3, t: np.ndarray = None) -> np.ndarray:
    ...
    d = len(v)
    if d != 4:
        raise ValueError("Currently supports exactly 4 variables.")
    ...
    return np.einsum('i,j,k,l -> ijkl', *basis_list).flatten()
...
class StateBS:
    ...
    eta: np.ndarray               # (n,)
    mu: float = 1.0               # global variance parameter for edges
...
class BN_LTE_MCMC_BSpline_Optimized:
    ...
    def __init__(
        ...
        *,
        ...
        self.n_timepoints = self.X.shape[0]  # Assuming each column is a timepoint
        ...
        eta0 = np.random.uniform(0.5, 1.5, 3)
        ...
        rho = self.h.pi_edge
        mu = self.h.mu
        self.state = StateBS(sigma2_0, beta0, gamma0, r0, z0, tau0, sigma3, rho, c0, eta0, mu)
        ...
        self.time_points = np.linspace(0, 1, 3)  # Normalized time points; adjust if needed
        ...
        self._basis_C = bspline_basis(np.linspace(0, 1, 3), K=self.K_beta, degree=self.degree)
        ...
        if len(var_indices) != 4:
            raise ValueError("Exactly 4 variable indices required.")
        self.var_indices = var_indices
        self.K_multi = self.K_beta
        self.degree_multi = 3
        self.t_multi = _make_t(self.K_multi, self.degree_multi)  # Reuse knot generation logic

        # Normalize the 4 variables to [0, 1]
        V = self.X[:, self.var_indices]  # Shape (n, 4)
        self.v_min = V.min(axis=0)
        self.v_max = V.max(axis=0)
        self.v_max[self.v_max == self.v_min] += 1e-6  # Avoid division by zero
        self.V_norm = (V - self.v_min) / (self.v_max - self.v_min)

        # Compute multivariate basis matrix (n, K_multi**4)
        K_total = self.K_multi ** 4
        self.basis_multi = np.zeros((self.n, K_total))
        for i in range(self.n):
            self.basis_multi[i] = tensor_product_basis_row(
                self.V_norm[i], K=self.K_multi, degree=self.degree_multi, t=self.t_multi
            )

        # ... existing code for z0, beta0, etc. ...

        # Initialize new state fields
        self.state.theta = np.zeros(K_total)
        self.state.sigma_z = st.invgamma.rvs(self.h.nu_sigma_z, scale=self.h.lambda_sigma_z)
    ...
    def _update_beta_and_r_blocked(self) -> None:
        ...
        X_products = {}
        for l in range(self.p):
            print('X shape:', self.X[:, l][:, None].shape)
            print('basis shape:', self._basis_beta.shape)
    ...
    def _update_beta(self) -> None:
        ...
        for l in range(self.p):
            ...
            print("basis shape:", self._basis_beta.shape)
            X_products[l] = self.X[:, l][:, None] * self._basis_beta
            print("X_product shape:", X_products[l].shape)
        for j in range(self.p):
    ...
    def _update_C(self):
        """Update C matrix (not used in current model)"""
        old_C = self.state.C.copy()
        eta = self.state.eta
        ...
        for p in range(3):
            for i in range(1, 3):
                for n in range(self.n):
                    C = bspline_basis_single_row(self.state.z[n], K=self.K_beta, t=self.t_delta)
                    print('C shape:', C)
                    print('state C shape:', self.state.C[p])
                    C_effect = self.state.C[p]
                    new_z = C_effect * self.X[n, p][:, None]
                    print('new_z shape:', new_z.shape)
                    print('new_z:', new_z)

                    b_new_beta = bspline_basis_single_row(new_z, K=self.K_beta, t=self.t_beta)
                    b_new_gamma = bspline_basis_single_row(new_z, K=self.K_gamma, t=self.t_gamma)

                    y_base = self.X_time[i, :, p]
                    
                    
                    for l in range(self.p):
                        D = self.X[:, l]
                        coeff = self.state.beta[p, l] * self.state.r[p, l]
                        if coeff.any():
                            D = D[:, None] * b_new_beta
                            D_effect = D @ coeff
                        y -= D @ C_effect
                    ...
                    baseline = b_new_gamma @ self.state.gamma.T
                    y -= baseline

                    Qt = D_act.T @ D_act / eta[p] + np.eye(1) / self.state.mu
                    L = np.linalg.cholesky(Qt)
                    m = np.linalg.solve(L.T, np.linalg.solve(L, D_act.T @ y)) / eta[p]
                    print('m shape:', m.shape)
                    print('Qt shape:', Qt.shape)
                    print('L shape:', L.shape)
                    new_C = st.multivariate_normal.rvs(mean=m, cov=np.linalg.inv(Qt), random_state=self.rng)
                    self.state.C[p] = new_C


    def _update_theta(self) -> None:
        """Update theta using multivariate normal."""
        sigma_z = self.state.sigma_z
        BtB = self.basis_multi.T @ self.basis_multi
        K_total = len(self.state.theta)
        Qt = BtB / sigma_z + np.eye(K_total) / self.h.tau_theta

        try:
            L = np.linalg.cholesky(Qt)
            vec = self.basis_multi.T @ self.state.z / sigma_z
            m = np.linalg.solve(L.T, np.linalg.solve(L, vec))
            self.state.theta = st.multivariate_normal.rvs(mean=m, cov=np.linalg.inv(Qt), random_state=self.rng)
        except np.linalg.LinAlgError:
            print("LinAlgError in _update_theta; skipping update.")

    
    def _update_sigma_z(self) -> None:
        """Update sigma_z using inverse gamma."""
        resid = self.state.z - self.basis_multi @ self.state.theta
        shape = self.h.nu_sigma_z + self.n * 0.5
        scale = self.h.lambda_sigma_z + 0.5 * np.sum(resid ** 2)
        self.state.sigma_z = np.clip(st.invgamma.rvs(a=shape, scale=scale), 1e-6, 1e6)
    ...
    #     """Update eta parameter using the inverse gamma distribution and residuals"""
    #     for i in range(self.p):
    #         resid = self.X[i] - self._baseline_contrib[i] - self._edge_contrib[i]
    #         shape = self.h.a_sigma + self.p / 2
    #         scale = self.h.b_sigma + 0.5 * np.sum(resid ** 2)
    #         self.state.eta[i] = st.invgamma.rvs(a=shape, scale=scale)
        
    #     pass
    def _update_eta(self):
        """Update eta as parameter-specific variances"""
        resid = self.X - self._baseline_contrib - self._edge_contrib  # shape: (n, p)
        
        for j in range(3):  # Loop over parameters/variables
            resid_j = resid[:, j]  # Residuals for variable j across all subjects
            shape = self.h.a_sigma + self.n / 2
            scale = self.h.b_sigma + 0.5 * np.sum(resid_j ** 2)
            self.state.eta[j] = st.invgamma.rvs(a=shape, scale=scale)
