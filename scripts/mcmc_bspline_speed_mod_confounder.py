...
@jit(nopython=True, cache=True)
def compute_residuals_fast(X: np.ndarray, baseline: np.ndarray, edges: np.ndarray, confounders: np.ndarray, no_incoming: np.ndarray) -> np.ndarray:
    residuals = X - baseline - edges - confounders
...
class StateBS:
    ...
    U: np.ndarray                 # (n, q) latent confounders
    Lambda: np.ndarray            # (p, q) loading matrix
...
class BN_LTE_MCMC_BSpline_Optimized:
    ...
    class Hyper:
        def __init__(
            ...
            a_sigma: float = 5.0,
            b_sigma: float = 5.0,
            # q: int = 3,               # Number of latent factors
            tau_lambda: float = 1.0   # Variance for Lambda priors
        ) -> None:
    ...
    def __init__(
        ...
        *,
        ...
        sigma2_0 = st.invgamma.rvs(a=self.h.a_sigma, scale=self.h.b_sigma, size=self.p)
        tau0 = np.full(self.p, 1.0)
        tau0 = st.gamma.rvs(a=1, scale=0.1, size=self.p)
        rho = self.h.pi_edge

        # New: Latent confounders
        self.q = q
        q = self.q
        U0 = np.random.normal(0, 1, size=(self.n, q))
        Lambda0 = np.random.normal(0, np.sqrt(self.h.tau_lambda), size=(self.p, q))

        self.state = StateBS(sigma2_0, beta0, gamma0, r0, z0, tau0, sigma3, rho, U0, Lambda0)
        ...
        self._confounder_contrib = np.zeros_like(self.X)  # New: U @ Lambda.T
        
        self._update_all_edge_contrib()
        self._update_baseline_contrib()
        self._update_confounder_contrib()
    ...
    def subinterval_cycle_check(
        ...
        check_points = np.linspace(0.2, 0.8, num_check_points)
    ...
    def _update_confounder_contrib(self) -> None:
        """New: Update confounder contributions"""
        self._confounder_contrib = self.state.U @ self.state.Lambda.T
    ...
    def _update_sigma2(self) -> None:
        """Vectorized sigma2 update - now uses updated residuals with confounders"""
        resid = compute_residuals_fast(self.X, self._baseline_contrib, self._edge_contrib, self._confounder_contrib, self._no_incoming)
    ...
    def _update_gamma(self) -> None:
        """Optimized gamma update with cached matrices and diagnostics - adjust for confounders"""
        ...
        for j in range(self.p):
            ...
            y = self.X[:, j] - self._edge_contrib[:, j] - self._confounder_contrib[:, j]
            tau_gamma = np.clip(self.state.tau_gamma[j], 1e-2, 1e4)
    ...
    def _update_tau2_mh(self) -> None:
        """Metropolis-Hastings update for global sigma_t^2 as per paper - adjust if needed for confounders"""
        rng = self.rng
        ...
        for j in range(self.p):
            ...
            resid = self.X[:, j] - self._baseline_contrib[:, j] - self._confounder_contrib[:, j]
            res2 = np.sum(resid**2)
            dr = D.T @ resid
    ...
    def _update_beta_and_r_blocked(self, it) -> None:
        """Optimized beta and r update - adjust y for confounders"""
        sigma2 = self.state.sigma2
        ...
        for j in rng.permutation(self.p):
            ...
            for l in rng.permutation(self.p):
                ...
                for k in rng.permutation(self.K_beta):
                    ...
                    y = self.X[:, j] - self._baseline_contrib[:, j] - self._edge_contrib[:, j] - self._confounder_contrib[:, j]
                    D_full = X_products[l]
                    sig2 = sigma2[j]
                    sig3 = sigma3
                    d = D_full[:, k]
    ...
    def _update_beta(self) -> None:
        ...
        for j in range(self.p):
            ...
            y_base = self.X[:, j] - self._baseline_contrib[:, j] - self._confounder_contrib[:, j]
            y = y_base.copy()
    ...
    def _update_z_single_row(self, it) -> None:
        ...
        for i in range(self.n):
            ...
            resid_old = self.X[i] - self._baseline_contrib[i] - self._edge_contrib[i] - self._confounder_contrib[i]
    ...
    def _update_latent_confounders(self):
        """New: Gibbs update for U and Lambda"""
        sigma2 = self.state.sigma2
        q = self.q
        tau_lambda = self.h.tau_lambda

        # Update Lambda (loadings) - for each j independently, treating as regression
        for j in range(self.p):
            if self._no_incoming[j]:
                self.state.Lambda[j, :] = 0.0
                continue
            y_j = self.X[:, j] - self._baseline_contrib[:, j] - self._edge_contrib[:, j]
            Qt = (self.state.U.T @ self.state.U) / sigma2[j] + np.eye(q) / tau_lambda
            try:
                L = np.linalg.cholesky(Qt)
                m = np.linalg.solve(L.T, np.linalg.solve(L, self.state.U.T @ y_j / sigma2[j]))
                self.state.Lambda[j] = st.multivariate_normal.rvs(mean=m, cov=np.linalg.inv(Qt), random_state=self.rng)
            except np.linalg.LinAlgError:
                continue

        # Update U (latents) - for each i independently, or vectorized
        inv_tau = np.diag(1.0 / sigma2)
        for i in range(self.n):
            y_i = self.X[i] - self._baseline_contrib[i] - self._edge_contrib[i]
            Qt = self.state.Lambda.T @ inv_tau @ self.state.Lambda + np.eye(q)
            try:
                L = np.linalg.cholesky(Qt)
                m = np.linalg.solve(L.T, np.linalg.solve(L, self.state.Lambda.T @ inv_tau @ y_i))
                self.state.U[i] = st.multivariate_normal.rvs(mean=m, cov=np.linalg.inv(Qt), random_state=self.rng)
            except np.linalg.LinAlgError:
                continue
    ...
    def run(self, n_iter: int = 2000, burnin: int = 500, thin: int = 5) -> List[StateBS]:
        """Run MCMC sampler with optimizations - add latent update"""
        draws: List[StateBS] = []
        ...
        for it in range(1, n_iter + 1):
            ...
            self._update_latent_confounders()  # New update step
